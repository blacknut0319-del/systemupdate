...
def open_guide_panel():
    ...
    add_w("펌업 버튼: 뚱USB(아두이노)에 최신 펌웨어를 한 번에 구워 넣음 (워치독+시리얼부트로더). 작업 중엔 정지 상태여야 함")
    add_w("확인 버튼: 연결된 뚱USB가 워치독 펌인지 조회 (응답 DDONG-WDT / DDONG-WDT2 이면 OK)")
...
PATCH_UPDATED_AT = "2026-07-28 18:40"
LATEST_PATCH = [
                    ...
                    def _ask():
                        try:
                            box["ok"] = bool(messagebox.askokcancel(
                                "자동 펌업 실패 (1회 수동)",
                                "지금 보드가 옛 워치독 펌이면 1200bps 자동리셋이 안 됩니다.\n"
                                "이번만 수동으로 WDT2(시리얼'!'지원)를 올리면,\n"
                                "이후부터는 리셋버튼 없이 자동 펌업됩니다.\n\n"
                                "보드에 【리셋 버튼】이 있으면:\n"
                                "1) 리셋 버튼을 빠르게 두 번 누르고\n"
                                "2) 바로 【확인】을 누르세요 (15초 안에 업로드)\n\n"
                                "리셋 버튼이 없고 옛 펌이면 ISP/다른 PC로\n"
                                "한 번만 WDT2를 올려야 합니다. 【취소】 후 로그 확인.",
                            ))
                        except Exception:
                            box["ok"] = True
                        ev.set()
                ...
                if wdt_ok:
                    ...
                    try:
                        messagebox.showinfo(
                            ...
                            "업로드 성공했습니다.\n"
                            "워치독 펌이 구워진 상태입니다.\n"
                            "원하면 몇 초 뒤 [확인]을 눌러 DDONG-WDT2 응답을 보세요.",
                        )
                    except Exception:
                        pass
            else:
