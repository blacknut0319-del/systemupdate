...
def _self_hp_pct_from_arr(arr, ref100=None):
    ...
    빨강뿐 아니라 독(초록)·석화(회색)도 채움으로 잡음 — 독이 초록을 채움으로 세는 것과 동일.
    회색은 밝기 하한을 빨강 채움(R>80)과 맞춰서, 어두운 빈칸 트랙이 회색으로 잡혀
    닳아도 100%로 고정되던 문제를 막음 (이전 R>20 기준이 빈칸까지 먹던 원인)."""
    if arr.size == 0:
        return 100.0
    try:
        ...
        # 석화 채움만 (독=초록처럼 색 조건으로 빈칸과 구분). 빈칸은 어두운 회색이라 R>80에서 탈락
...
LATEST_PATCH = [
    "🪨 석화 HP% — 독(초록)과 같이 회색 채움도 %에 포함. 다만 이전 회색기준(R>20/30)이 빈칸 트랙까지 먹어서 빨간피도 닳아도 100%로 고정되던 버그 → 밝기 R>80(빨강 채움과 동일)으로 조여 빈칸 제외. 쫄법·격수모니터 동일",
