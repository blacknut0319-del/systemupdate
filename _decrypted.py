...
def _self_hp_fill_masks(R, G, B):
    ...
    치명적 버그: 절대 밝기(mx>60)만으로 판정했더니, 빈 피통(안 채워진 트랙)이 검은색이
    아니라 어두운 빨강/자주색 트랙으로 그려지는 UI에서는 그 어두운 트랙까지 전부
    '채움'으로 잡혀서, 실제 피가 얼마나 빠지든 바 전체가 항상 100%로 고정되어버렸음
    (실제 전투로 피가 빠져도 위기베르가 전혀 반응 안 하던 근본 원인).
    해결: ROI 안에서 가장 밝은 픽셀 대비 상대적으로 밝은 픽셀만 '진짜 채움'으로 인정
    (어두운 빈 트랙은 밝은 실제 채움보다 확실히 어두우므로 상대비교로 걸러짐)."""
    mx = np.maximum(np.maximum(R, G), B)
    mn = np.minimum(np.minimum(R, G), B)
    peak = float(mx.max()) if mx.size else 0.0
    bright_ok = mx >= max(60.0, peak * 0.6)
    return bright_ok & ((mx - mn) > 40) & (B < mx - 15)
...
PATCH_UPDATED_AT = "2026-07-16 15:02"
LATEST_PATCH = [
