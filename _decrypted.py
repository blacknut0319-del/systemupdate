...
def _self_hp_fill_cols(arr):
    ...
    if int(rows.max()) >= 2:
        ...
        mask = mask[lo:hi + 1]
    col = mask.any(axis=0)
    gap_tol = 2   # 진짜 이어진 바만 인정 — 폭 비례(w//20)로 두면 중앙 숫자와의 틈을
                  # 뛰어넘어 이어붙여서, 피가 얼마나 빠지든 숫자 위치까지의 값으로
                  # 고정되어버리는(예: 40% 밑으로 안 내려감) 문제가 있었음. 안티앨리어싱
                  # 정도의 아주 작은 틈만 허용해 실제로 끊긴 별개 덩어리는 절대 안 이어붙임.
    idx = np.nonzero(col)[0]
    if idx.size == 0:
        return 0, w
